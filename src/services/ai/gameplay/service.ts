  ...
  async generateStoryTurn(
    ...
    try {
      ...
      if (lastAssisMsg && lastAssisMsg.text) {
          ...
          const snippet = rawStory.trim().slice(-400); // Increased slice size
          extraUserInject += `\n\n<SHORT_TERM_MEMORY>\nYour last response ended exactly like this:\n"...${snippet}"\n⚠️ CRITICAL AMNESIA FIX: Immediately resume action FROM THIS EXACT STATE. Maintain same held items, weapons, and body posture from your last paragraph. Do NOT drop continuity!\n</SHORT_TERM_MEMORY>`;
      }
